...
def getinputdict(loglineareqs = True):
    ...
    inputdict['controls'] = ['C', 'Rp', 'W', 'L', 'Y', 'MC', 'Omega', 'I', 'Pi', 'U', 'V', 'PstaroverP', 'rk']
    inputdict['shocks'] = ['epsilon_A', 'epsilon_I']
    ...
    if loglineareqs is True:
        ...
        inputdict['equations'].append('I = Rp * Pi_p')
    if loglineareqs is True:
        inputdict['equations'].append('Rp = rk_ss / (rk_ss + 1 - DELTA) * rk_p')
    else:
        inputdict['equations'].append('Rp = rk_p + 1 - DELTA')
    if loglineareqs is True:
    ...
    # firm production
    if loglineareqs is True:
        inputdict['equations'].append('MC = rk - A + (1 - ALPHA) * K - (1 - ALPHA) * L')
    else:
        inputdict['equations'].append('MC = rk / (ALPHA * A * K**(ALPHA-1) * L**(1-ALPHA) )')
    if loglineareqs is True:
    ...
    k = (p['MC'] * p['ALPHA'] * p['A'] / p['rk']) ** (1 / (1 - p['ALPHA']))
    ...
    inputdict['irfshocks'] = ['epsilon_A', 'epsilon_I']

    # save stuff
    inputdict['savefolder'] = __projectdir__ / Path('temp/')

    # main vars
    inputdict['mainvars'] = ['C', 'Rp', 'Pi', 'I', 'K', 'L']
